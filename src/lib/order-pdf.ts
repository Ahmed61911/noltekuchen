...
const STATUS_CHIP: Record<string, { label: string; bg: number[]; fg: number[] }> = {
  pending:   { label: "En attente",    bg: [254, 243, 199], fg: [146, 64, 14] },
  validated: { label: "Validée",       bg: [219, 234, 254], fg: [29, 78, 216] },
  delivered: { label: "Livrée",        bg: [209, 250, 229], fg: [21, 128, 61] },
  cancelled: { label: "Annulée",       bg: [254, 226, 226], fg: [185, 28, 28] },
};

const PAYMENT_TERMS = [
  "Validité de l'offre : 30 jours à compter de la date d'émission.",
  "La signature de ce bon de commande vaut acceptation des conditions générales de vente.",
];
...
export function generateOrderPdf(inv: PdfOrder) {
  ...
  const drawRunningHeader = () => {
    ...
    doc.text(pdfText(`Commande ${inv.order_number} — suite`), pageW - M, 20, { align: "right" });
  ...
  doc.setFontSize(19);
  doc.setTextColor(...INK);
  doc.text("BON DE COMMANDE", pageW - M, 19, { align: "right" });

  const metaLabelX = pageW - M - 62;
  const metaValueX = pageW - M;
  const meta: Array<[string, string]> = [
    ["N° de commande", pdfText(inv.order_number)],
    ["Date d'émission", formatDate(inv.order_date)],
    ["Date de validité", formatDate(inv.due_date)],
  ];
  ...
  doc.setFontSize(7);
  doc.setTextColor(...MUTED);
  const amountLabel = "MONTANT DE LA COMMANDE";
  ...
  // Les colonnes optionnelles disparaissent quand elles n'apportent rien :
  ...
  const fileName = String(inv.order_number ?? "bon-de-commande").replace(/[\\/:*?"<>|]+/g, "-");
