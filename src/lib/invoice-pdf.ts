...
export function generateInvoicePdf(inv: PdfInvoice) {
  ...
  const drawRunningHeader = () => {
    ...
    doc.text(pdfText(`Facture ${inv.invoice_number} — suite`), pageW - M, 20, { align: "right" });
  ...
  doc.setFontSize(19);
  doc.setTextColor(...INK);
  doc.text("FACTURE", pageW - M, 19, { align: "right" });

  const metaLabelX = pageW - M - 62;
  const metaValueX = pageW - M;
  const meta: Array<[string, string]> = [
    ["N° de facture", pdfText(inv.invoice_number)],
    ["Date d'émission", formatDate(inv.invoice_date)],
  ...
  doc.setFontSize(7);
  doc.setTextColor(...MUTED);
  tracked(doc, pdfText("FACTURÉ À"), M + 6, blockY + 7, 0.35);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(11);
  doc.setTextColor(...INK);
  ...
  doc.setFontSize(7);
  doc.setTextColor(...MUTED);
  const amountLabel = inv.status === "paid" ? "MONTANT RÉGLÉ" : "MONTANT À PAYER";
  ...
  // ventes comptoir n'ont pas de référence article, et la plupart des factures
  // n'ont aucune remise ligne à ligne.
  const hasCode = inv.items.some((it) => String(it.code ?? "").trim() !== "");
  const hasDiscount = inv.items.some((it) => Number(it.discount_rate) > 0);
  ...
  const cell = (col: Col, it: PdfInvoice["items"][number]) => {
    switch (col.key) {
      ...
      case "rem": {
        const rate = Number(it.discount_rate) || 0;
  ...
  doc.setFontSize(8);
  doc.setTextColor(...INK);
  tracked(doc, pdfText("CONDITIONS DE PAIEMENT"), M, termsY, 0.35);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(7.5);
  doc.setTextColor(...MUTED);
  ...
  const fileName = String(inv.invoice_number ?? "facture").replace(/[\\/:*?"<>|]+/g, "-");
